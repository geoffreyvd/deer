...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000, batch_size=32, update_rule="rmsprop", random_state=np.random.RandomState(), double_Q=False, neural_network=NN, **kwargs):
        ...
        self.loss_interpret=0 #enforcing a direction
        self.loss_T=0
        self.lossR=0
        self.loss_Q=0
        self.loss_disentangle_t=0
        self.loss_disambiguate1=0 #entropy maximization (two different states should have different abstract states) this makes it work even without a reward
        self.loss_disambiguate2=0 #keep everything in a radius of 1
        self.loss_gamma=0


        #memorize losses to plot
        self.lossR_list=[]
        self.loss_gamma_list=[]
        self.loss_Q_list=[]
        self.loss_T_list=[]

        self.loss_disentangle_t_list=[]
        self.loss_disambiguate1_list=[]
        self.loss_disambiguate2_list=[]
        # self.loss_interpret_list=[]
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        if(self.update_counter%UPDATE_AFTER_STEPS==0):
            ...
            loss_gammaAvg = self.loss_gamma/UPDATE_AFTER_STEPS
            loss_QAvg = self.loss_Q/UPDATE_AFTER_STEPS
            loss_TAvg = self.loss_T/UPDATE_AFTER_STEPS

            loss_disentangle_tAvg = self.loss_disentangle_t/UPDATE_AFTER_STEPS
            loss_disambiguate1Avg = self.loss_disambiguate1/UPDATE_AFTER_STEPS
            loss_disambiguate2Avg = self.loss_disambiguate2/UPDATE_AFTER_STEPS
            # loss_interpretAvg = self.loss_interpret/UPDATE_AFTER_STEPS
            ...
            print (loss_TAvg, lossRAvg,loss_gammaAvg, loss_QAvg, loss_disentangle_tAvg, loss_disambiguate1Avg, loss_disambiguate2Avg)
            ...
            self.loss_gamma_list.append(loss_gammaAvg)
            self.loss_Q_list.append(loss_QAvg)
            self.loss_T_list.append(loss_TAvg)

            self.loss_disentangle_t_list.append(loss_disentangle_tAvg)
            self.loss_disambiguate1_list.append(loss_disambiguate1Avg)
            self.loss_disambiguate2_list.append(loss_disambiguate2Avg)
            # self.loss_interpret_list.append(loss_interpretAvg)

            #TODO plot that shit
            plt.plot(range(1, len(self.lossR_list)+1), self.lossR_list, label="R", color='b')
            plt.plot(range(1, len(self.lossR_list)+1), self.loss_gamma_list, label="gamma", color='g')
            plt.plot(range(1, len(self.lossR_list)+1), self.loss_Q_list, label="Q", color='r')
            plt.plot(range(1, len(self.lossR_list)+1), self.loss_T_list, label="T", color='c')
            plt.plot(range(1, len(self.lossR_list)+1), self.loss_disentangle_t_list, label="disentangle_t", color='m')
            plt.plot(range(1, len(self.lossR_list)+1), self.loss_disambiguate1_list, label="disambiguate1", color='y')
            plt.plot(range(1, len(self.lossR_list)+1), self.loss_disambiguate2_list, label="disambiguate2", color='k')
