...
class MyEnv(Environment):
    ...
    def create_map(self):
        ...
        self._pos_agent=[2,2]
        self._pos_goal=[self._size_maze-3,self._size_maze-6]

        # #TODO add rewards
        # self._pos_rewards=[[x,y]]

        # #on those locations indicate a key on the map with a 2
        # self._map[_pos_rewards[0]]=2
        # self._map[_pos_rewards[1]]=2
        # self._map[_pos_rewards[2]]=2
    ...
    def act(self, action):
        ...
        if (self._pos_agent[0] == self._pos_goal[0] and self._pos_agent[1] == self._pos_goal[1]):
            ...
            # self._pos_goal = [] #maybe dont remove it, while just learning the reward model
    ...
    def summarizePerformance(self, test_data_set, learning_algo, *args, **kwargs):
        ...
        all_possib_inp=[] # Will store all possible inputs (=observation) for the CRAR agent 31 states
        labels_maze=[]
        self.create_map()
        ...
        print("--- len and shape ---")
        print(len(all_possib_abs_states))
        try:
            print(all_possib_abs_states.shape)
        except:
            pass
            
        print(" --- all abstract states ---")
        print(all_possib_abs_states
        )
        #TODO predict reward
        x = np.array(all_possib_abs_states)[:,0]
        y = np.array(all_possib_abs_states)[:,1]

        amountOfStates = len(x)
        actions_val = []
        onehot_actions = np.zeros((amountOfStates, 4))
        # onehot_actions[np.arange(self._batch_size), actions_val] = 1
        onehot_actions[:, 0] = 1

        print("--- one hot actions ---")
        print(onehot_actions)
        print(len(onehot_actions))
        ...
        if(abs_states.ndim==4): #should i remove this?
